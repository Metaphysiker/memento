...
RSpec.describe "tags", :type => :feature do
  ...
  before(:each) do
    ...
    person_functionality_tags.each do |tag|
      ...
      institutions_target_groups_tags = ["Philosophisches Institut(Zielgruppe)", "Kooperationspartner(Zielgruppe)","SPG/SAGW(Zielgruppe)",
              "Stiftung(Zielgruppe)", "Philosophischer Verein(Zielgruppe)", "Verlag(Zielgruppe)", "Sponsor(Zielgruppe)", "öffentliche Institution(Zielgruppe)",
              "Unternehmen(Zielgruppe)", "Medienkontakt(Zielgruppe)"]
